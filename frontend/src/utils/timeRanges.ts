...
export const DEFAULT_TIME_RANGE: TimeRangeKey = "last_30_days";
...
export function toIsoWithOffset(dt: Date): string {
...
function quarterIndex(monthZeroBased: number) {
  return Math.floor(monthZeroBased / 3);
}

function quarterStart(year: number, qIndex: number) {
  return new Date(year, qIndex * 3, 1, 0, 0, 0, 0);
}

function quarterEnd(year: number, qIndex: number) {
  return new Date(year, qIndex * 3 + 3, 0, 23, 59, 59, 999);
}

function lastQuarterRange(now: Date) {
...
export function isTimeRangeKey(value: string | null | undefined): value is TimeRangeKey {
